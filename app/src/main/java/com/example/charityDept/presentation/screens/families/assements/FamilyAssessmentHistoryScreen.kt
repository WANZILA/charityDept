...
fun FamilyAssessmentHistoryScreen(
    ...
    val emptyToolsText = if (isObservationMode) {
        "No active family observation tools found yet."
    } else {
        "No active family Q&A tools found yet."
    }

    val emptyHistoryText = if (isObservationMode) {
        "No family observation sessions yet. Tap + to start one."
    } else {
        "No family Q&A sessions yet. Tap + to start one."
    }

    val startButtonLabel = if (isObservationMode) "Start Observation" else "Start Q&A"

    val sessionsFlow = remember(familyId, mode) { vm.sessions(familyId, mode) }
            ...
            confirmButton = {
                TextButton(
                    ...
                    onClick = {
                        ...
                        vm.startNewAssessment(familyId, mode) { newId ->
